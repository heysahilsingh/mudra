...
import { dbAppSettings, dbWallets, resetDb } from '../../db.js';
import { appendHTML, createHTML, popMsg, updateAppColors, updateAppUserCurrency } from '../../helper.js';
...
export function pageSettings() {
    ...
    const body = () => {
        ...
        const sectionUserProfile = () => {
            ...
            userNameInput.addEventListener("input", () => {
                if (userNameInput.value.trim() === '') {
                    ...
                    console.log(dbAppSettings.userName)
                }
            })
        ...
        const sectionPreferences = () => {
            ...
            // Default Screen
            const defaultScreen = () => {
                const setting = createHTML("div", { class: "setting arrow default-screen" });
                const settingIcon = createHTML("i", { class: "setting-icon ph-fill ph-grid-four" });
                const settingName = createHTML("p", { class: "setting-name" }, "Deafult screen");
                const settingController = createHTML("div", { class: "setting-controller" }, "Home");

                setting.addEventListener("click", () => {
                    compConfirmDialogBox({
                        pBoxId: "page-settings",
                        pMessage: createHTML("p", {}, "sasasasas"),
                        pButton2: "Ok"
                    })
                })
            ...
            // Main Appending
            appendHTML([editCategories(), defaultWallet(), currency(), defaultScreen()], sectionContainer)
