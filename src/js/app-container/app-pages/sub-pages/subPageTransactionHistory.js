...
import { appendHTML, closeSubPage, consoleError, createHTML, formatAmount, getDates, getTransactions } from "../../../helper.js";
  ...
  if (document.querySelector(".app .app-page")) {
    ...
    const pageWrapper = (() => {
      ...
      const body = (() => {
        ...
        let dataDates =
          pDates ||
          getDates(new Date("09/30/2023"), dbAppSettings.firstTransaction);
        ...
        function renderData() {
          ...
          for (let date of dataDates) {
            const row = compTransactionsRow({
              pDate: date,
              pTransactions: dataTransactions,
              pCallback: () => {
                renderData()
              }
            });
          ...
          netBalance.incomeData.textContent = `${formatAmount(newIncome)} ${dbAppSettings.userCurrency
            }`;
          netBalance.expenseData.textContent = `${formatAmount(newExpense)} ${dbAppSettings.userCurrency
            }`;
