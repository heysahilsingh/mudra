...
*/
  ...
  if (document.querySelector(".app .app-page")) {
    ...
    // Sub Page
    const page = createHTML("div", { class: "sub-page parent-category-picker" });
    ...
    const pageWrapper = (() => {
      ...
      const header = (() => {
        ...
        const heading = createHTML("p", { class: "page-heading" }, "Select Parent Category");
      ...
      const body = (() => {
        ...
        const parentCategories = dbCategories[pCategoryType].filter(category => !category.isChild);

        // Helper Function - to create category HTML
        const createCategoryElement = (category) => {
          const categoryElement = createHTML("li", { class: `category icon ${category.icon}`, });
          const categoryName = createHTML("li", { class: "category-name" }, category.name);
          const selectedCategoryIcon = createHTML("i", { class: "ph-fill ph-check-circle", });

          // Add Event Listener
          categoryElement.addEventListener("click", () => {
            // Remove the "selected" class from any existing <li> elements
            const selectedCategory = body.querySelectorAll(".category.selected");

            if (selectedCategory.length > 0) selectedCategory.forEach(ele => ele.classList.remove("selected"));

            categoryElement.classList.add("selected");

            // Run pCallback
            if (pCallback && typeof pCallback === "function") pCallback(category);

          });

          // Preselected
          if (pPreSelected && pPreSelected === category.id) categoryElement.click();
        ...
        const sectionNoCategory = () => {
          const sectionNoCategory = createHTML("section", { class: "section-no-category" });

          const category = createHTML("li", { class: `category ${!pPreSelected ? "selected" : ""}`, });
          const categoryName = createHTML("li", { class: "category-name" }, "Not selected");
          const selectedCategoryIcon = createHTML("i", { class: "ph-fill ph-check-circle" });
        ...
        const sectionParentCategories = () => {
          const sectionParentCategories = createHTML("section", { class: "section-parent-categories" });
