...
import { dbCategories, dbTransactions, dbUserDetails, dbWallets, } from "./db.js";
  ...
  */
  ...
  try {
    ...
    if (transaction) {
      ...
      const transactionAmount = createHTML("span", { class: "transaction-amount" }, `${formatAmount(transaction.amount)} ${dbUserDetails.currency}`);
  ...
  try {
    ...
    if (transactionsArray.length > 0) {
      ...
      const data = (() => {
        ...
        const sortedTransactionsArray = sortTransaction(transactionsArray, "date");
      ...
      const handler = (() => {
        ...
        const totalAmount = (() => {
          ...
          if (totalIncome > 0) {
            const tIncome = createHTML("p", { class: "total-income" }, `${formatAmount(totalIncome)} ${dbUserDetails.currency}`);
            amount.push(tIncome);
          }

          if (totalExpense > 0) {
            const tExpense = createHTML("p", { class: "total-expense" }, `${formatAmount(totalExpense)} ${dbUserDetails.currency}`);
...
export function compConfirmDialogBox(pObject) {
  ...
  const { pMessage, pButton1, pButton2, pCallback } = pObject;

  // Wrapper
  const wrapper = createHTML("div", { class: "confirm-dialog-box" });
  ...
  const button1 = createHTML("button", { class: "button-1 btn btn-red" }, pButton1 ? pButton1 : "Cancel");
  const button2 = createHTML("button", { class: "button-2 btn" }, pButton2 ? pButton2 : "Ok");
