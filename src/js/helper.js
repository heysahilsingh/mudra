...
// FUNCTION - SA_ERROR
export function saError(err, msg) {
  console.error(`SA_ERROR: ${msg} ${err.stack}`);
}

// FUNCTION - CONSOLE ERROR
export function consoleError(msg) {
  console.error(`SA_ERROR: ${msg}`);
}

// FUNCTION - GET ICON URL
export function iconUrl(iconName) {
  return `.././media/icons/${iconName}.png`;
}
...
export function sortTransaction(array, sortBy) {
  ...
  newArray.sort((a, b) => {
    const valueA =
      sortBy === "date" ? formatDate(a[sortBy]).saveDate : parseInt(a[sortBy]);
    const valueB =
      sortBy === "date" ? formatDate(b[sortBy]).saveDate : parseInt(b[sortBy]);
